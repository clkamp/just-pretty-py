...
class JustExpTransformer(Transformer):
    ...
    def expression_def(self, definition):
        ...
        return {
            name: order_entries(
                body, ["doc", "vars", "vars_doc", "imports", "expression"]
            )
        }
    ...
    def function(self, arg):
        ...
        keyword_args = list(
            reversed(list(map(lambda x: {x.children[0]: x.children[1]}, keyword_args)))
        )
        exprs = list(filter(lambda x: not isinstance(x, Tree), arg[1:]))
        exprs = list(
            map(lambda a: {f"${a[0] + 1}": a[1]}, zip(range(len(exprs)), exprs))
        )
    ...
    def for_map(self, k, v, r, body):
        ...
        return order_entries(
            dict(**{"type": "foreach_map", "range": r, "body": body}, **k, **v),
    ...
    def foldl(self, r, start, acc, v, body):
        ...
        return order_entries(
            dict(**{"type": "foldl", "range": r, "body": body}, **acc, **v, **start),
    ...
    def expr_doc(self, docs):
        vars_doc = dict(
            reversed(
                dict(
                    ChainMap(*[d["vars_doc"] for d in docs if "vars_doc" in d])
                ).items()
            )
        )
        return dict(ChainMap(*docs), **({"vars_doc": vars_doc} if vars_doc else {}))
    ...
    def cond(self, clauses, default):
        return dict(**{"type": "cond", "cond": clauses}, **default)
...
def tokenize(t):
    if isinstance(t, dict):
        ...
        if type == "singleton_map":
            ...
            start = tt(
                "foldl_start", [tokenize_exp(t["start"])] if "start" in t else []
            )
            acc_var = tt(
                "for_map_bind",
                [tokenize_name(t["accum_var"])] if "accum_var" in t else [],
            )
            ...
            range = tokenize_exp(t["range"])
            key = tt(
                "for_map_bind", [tokenize_name(t["var_key"])] if "var_key" in t else []
            )
            val = tt(
                "for_map_bind", [tokenize_name(t["var_val"])] if "var_val" in t else []
            )
            body = tokenize_exp(t["body"])
            ...
            body = [tokenize_exp(t["body"])]
            binding = lambda b: tt(
                "let_bind", [tokenize_name(b[0]), tokenize_exp(b[1])]
            )
            ...
            cond_clauses = (
                [tt("cond_clauses", [cond_clause(k, v) for k, v in t["case"].items()])]
                if "case" in t
                else []
            )
            default = [
                tt(
                    "optional_default",
                    [tokenize_exp(t["default"])] if "default" in t else [],
                )
            ]
            return tt("case", expr + cond_clauses + default)
        elif type == "case*":
            expr = [tokenize_exp(t["expr"])] if "expr" in t else []
            cond_clause = lambda c: tt(
                "clause", [tokenize_exp(c[0]), tokenize_exp(c[1])]
            )
            cond_clauses = (
                [tt("cond_clauses", [cond_clause(c) for c in t["case"]])]
                if "case" in t
                else []
            )
            default = [
...
def tokenize_target(t):
    if isinstance(t, str):
        ...
        return Tree(
            "qualified_target", [Token("ESCAPED_STRING", json.dumps(s)) for s in t]
        )
...
def tokenize_expr_def(e, name):
    ...
    expr_vars = (
        [
            tt("expr_doc_var", [tokenize_name(n), expr_var(v)])
            for n, v in e["vars_doc"].items()
        ]
        if "vars_doc" in e
        else []
    )
    ...
    expr_args = tt(
        "exp_args", [tokenize_name(x) for x in e["vars"]] if "vars" in e else []
    )
    ...
    usings = (
        [
            tt(
                "using_block",
                [
                    tt("using", [tokenize_name(k), tokenize_target(v)])
                    for k, v in e["imports"].items()
                ],
            )
        ]
        if "imports" in e
    ...
    for item in items:
        if not printed_newlines:
            if item == "exp":
                ...
                isinstance(item, Token) and item.type == "EXPR_DOC_MAIN"
            ) or item == "*v ":
                yield "\n\n"
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1].startswith("e"):
            print(
                hdumps(JustExpTransformer().transform(parser.parse(sys.stdin.read())))
            )
        else:
